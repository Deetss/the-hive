 ...
 * `/releases/latest/download/The-Hive-<version>-<platform>.<ext>`. That URL
...
const assetRe = /The-Hive-(\d+\.\d+\.\d+)-([^\s`)]+)/g;
const assets = new Set();
for (const m of releaseMd.matchAll(assetRe)) {
  if (m[1] !== version) {
    problems.push(`RELEASE.md advertises The-Hive-${m[1]}-${m[2]} but package.json says ${version}`);
  }
  assets.add(`The-Hive-${m[1]}-${m[2]}`);
}
...
async function checkLive() {
  const base = 'https://github.com/Deetss/the-hive/releases/latest/download/';

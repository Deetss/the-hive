...
- Mobile PWA setup now autofills all three fields (URL, secret, label) from a scanned QR, honors optional `?url=`/`?label=` params, and remembers the last setup so a bookmark or home-screen open is prefilled instead of blank

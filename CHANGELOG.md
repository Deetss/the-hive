...
- Settings → Memory & Knowledge: the knowledge base is now a LIST of sources you can add to freely, each a "Local Folder" (`.md`/`.txt` agents grep/read), "Outline MCP" (defaults to the team Outline URL, editable), or "Custom MCP URL". Agents fan out across every source; each https MCP entry is named in the orientation and, for Claude agents, wired into the per-session `mcpServers` as `hive-kb-1`, `hive-kb-2`, …. Persisted as `knowledgeBaseSources` in config and over `/api/settings`; the old single-folder setting is migrated on read
...
- Shared knowledge base: point TheHive at a local folder of `.md`/`.txt` notes (Settings → Memory & Knowledge → Knowledge base folder); its path is injected into every agent's orientation so they grep/read it for team knowledge, and `kb:list`/`kb:read`/`kb:search` IPC (path-contained to the folder) lets the app browse it
- Mobile PWA: Respawn (↺) button on agent fleet cards (archives the session and restarts from memory.md), backed by a new `POST /api/agents/:id/respawn` route that shares the desktop respawn logic
- Mobile PWA: raw terminal input under the agent terminal — a text field (Enter submits) plus Ctrl+C / Esc / Tab / arrows / Enter keys, written straight to the agent's PTY via a new `POST /api/agents/:id/input` route
- Mobile PWA fleet cards now show the desktop parity chips: `⊘ quota` (quota-limited), `idle Xh` (idle over 2h), and a model chip, from the existing `fleet.json` fields
- Mobile PWA settings screen (⚙): set the floor token cap, toggle the governor force-green override, pick the default model for new agents, and set/clear the knowledge-base folder, backed by new `GET`/`POST /api/settings` routes
- Mobile PWA: New Agent form in the Workers tab — name, worker/harness type, runtime-profile dropdown, and optional objective — posts to `POST /api/agents/spawn` (now honoring a `profile` id) and the agent appears in Fleet via the SSE update
- Mobile PWA: Schedules screen (⏰ in the header) lists scheduled auto-dispatch missions with their cadence, target, last-fired and next-fire time, toggles each on/off, and creates a new interval mission, backed by new `GET /api/missions`, `POST /api/missions/:id/toggle`, and `POST /api/missions` routes
- Onboarding wizard now has a skippable Knowledge Base step (STEP 4 OF 5): add local `.md`/`.txt` folders and/or Outline/custom MCP doc sources agents fan out across, so new users configure the shared KB during setup instead of discovering it later; reuses the SettingsModal `knowledgeBaseSources` pattern
...
- Agent guidance now makes surfacing questions to the human unambiguous: PROTOCOL.md and the per-agent orientation tell agents that ANY question needing the human (a quick yes/no included) must go on the ASK ME board via a `humanQA` card, never only in their terminal output where the human has to scroll to find it (a non-blocking question can stay `doing` while the card is posted)
...
- Composer attachments now clearly take any file type (the OS picker defaults to "All Files"), and each attachment chip shows its size; attached paths ride along in both a queued message and a dispatch so the receiving agent can Read them
...
- Closing time now runs even when the usage governor is RED — a governor-paced pause used to deny every tool call for paced workers, so they could never commit their WIP or send `CLOSING-TIME-ACK` and the graceful shutdown hung. The pause is exempted for the duration of the closing-time protocol (an explicit operator tool-gate still applies)

...
1. Set the task's \`status\` to \`done\` in \`tasks.json\` (keep its \`assignee\` and \`progressLog\` — a done card still says who did the work).
2. Send a \`done\` report to Abathur (\`"to":"god"\`) with the concrete result: commit hash, files changed, what changed, and how you verified it. Never a bare "done".
3. **Re-drain your \`inbox/\`.** New dispatches and \`urgent\` messages routinely arrive while you were heads-down. Process \`urgent\` first, then FIFO, and move each to \`.done/\`.
4. If nothing is waiting, scan \`tasks.json\` for an unassigned \`todo\` whose dependencies are met and that fits your skills. Claim it (set \`assignee\` to your id + \`status\` to \`doing\`) and start — you do not need to wait for a hand-off.
5. Only if there is genuinely nothing to pick up, send Abathur a one-line \`inform\`: \`"inbox empty, idle and available"\`. Then stop. Do NOT sit polling your own session — the harness will wake you when mail arrives.

...
/** One shared knowledge-base source (mirrors config.ts KnowledgeBaseSource;
 *  kept local so hive.ts never imports the config module just for a type). */
type KbSource = { type: 'folder' | 'outline-mcp' | 'custom-mcp'; value: string };
...
export interface RegistryAgent extends AgentMeta {
  ...
  archived?: boolean;
  /** The human has this agent 1:1 and Abathur must leave it alone until they
...
/** User-editable prompt templates (config.promptOverrides), mirrored onto the
 *  Hive instance. Each is the static behavioral body of an agent role's system
 *  prompt; blank/absent falls back to the shipped default. The identity header
 *  and computed feature/path lines around each body are never overridable. */
export interface PromptOverrides {
  /** Standing behavioral block for plain workers (guardrails, progress, ASK ME gate, close-out). */
...
export class HiveManager {
  ...
  private _promptOverrides: PromptOverrides = {};
  setPromptOverrides(overrides: PromptOverrides | undefined): void {
    this._promptOverrides = overrides ?? {};
  }

  /** The shipped defaults for the user-editable prompts — for the Settings UI to
   *  pre-fill and revert-to-default. workerOrientation is the standard worker
   *  orientation body; overmindOrientation is the Overmind's orchestration body;
   *  protocolTemplate is the PROTOCOL.md template. */
  promptDefaults(): Required<PromptOverrides> {
    return {
  ...
  ensureHive(): void {
    ...
    // The Claude Code command reference Abathur consults (refreshed each bootstrap
  ...
  async ensureAgent(
    ...
    opts: {
      ...
      kgCliPath?: string;
      /** Shared knowledge-base sources (config, migrated). Each is a local
       *  `.md`/`.txt` folder or an https MCP endpoint. The agent's orientation
       *  names ALL of them; for Claude agents every https MCP entry is also wired
       *  into the per-session mcpServers as `hive-kb-<n>`. */
    ...
    const role = preferredAgentRole(meta.role, prev?.role, !!meta.isOvermind);
    meta = { ...meta, role };
    ...
    if (meta.isOvermind) reg.godId = meta.id;
    this.atomicWriteJson(join(root, 'registry.json'), reg);

    this.appendLog({ kind: 'spawn', agentId: meta.id, name: meta.name, isOvermind: !!meta.isOvermind });
    ...
    if (!isHiveAwareProvider(meta.provider)) {
      ...
      const prompt = this.injectedPrompt(meta, dir, root, opts.semanticMemory ?? false, opts.knowledgeGraph ?? false, opts.kgCliPath, opts.knowledgeBaseSources);
    ...
    args.push('--append-system-prompt', this.injectedPrompt(meta, dir, root, opts.semanticMemory ?? false, opts.knowledgeGraph ?? false, opts.kgCliPath, opts.knowledgeBaseSources));
   ...
   */
  /**
   * Put an agent on hold, or take it off, and tell Abathur immediately.
   *
  ...
  private buildDefaultMcpServers(
    ...
    // Shared-knowledge-base MCP sources (config.knowledgeBaseSources entries of
    // type outline-mcp / custom-mcp, in order). Each https URL becomes an
    // HTTP-transport server `hive-kb-<n>` so the agent has a real tool; the
  ...
  private identityText(meta: AgentMeta): string {
    ...
    return [
      `# ${meta.name} (${meta.id})`,
      '',
      `- Role: ${meta.role ?? (meta.isOvermind ? 'Overmind' : 'agent')}`,
      `- Capabilities: ${caps}`,
      `- Working directory: ${meta.cwd}`,
      meta.isOvermind ? '- You are the **Overmind / orchestrator**. You run the floor — keep awareness of the whole team, delegate execution, and personally own only the important calls (decomposition, sign-offs, conflicts, integration), not the grunt work.' : '',
      meta.isOvermind ? '- Monitor the team with `fleet.json` (live per-agent status/tokens/cost/breaker) and `registry.json`; full command reference in `COMMANDS.md`. `claude agents` does NOT list your hive siblings.' : '',
      ''
    ].filter(Boolean).join('\n');
  }
  ...
  private injectedPrompt(
    ...
    // ALL of them. Outline gets its `search_documents`/`get_document` verbs named;
    // a custom MCP stays generic. Each https MCP entry is wired (Claude agents)
    // as `hive-kb-<n>`, numbered by its position among the MCP entries.
    const kbFolders = (knowledgeBaseSources ?? []).filter((s) => s.type === 'folder').map((s) => s.value);
    const kbMcps = (knowledgeBaseSources ?? []).filter((s) => s.type === 'outline-mcp' || s.type === 'custom-mcp');
    const kbParts: string[] = [];
    if (kbFolders.length) {
      kbParts.push(
        kbFolders.length === 1
          ? `grep/Read the folder ${kbFolders[0]}`
          : `grep/Read these folders: ${kbFolders.join(', ')}`
      );
    }
    kbMcps.forEach((s, i) => {
      const server = `hive-kb-${i + 1}`;
      kbParts.push(
        s.type === 'outline-mcp'
          ? `query the Outline workspace at ${s.value} via the \`${server}\` MCP server (\`search_documents\`, then \`get_document\`)`
          : `query the MCP server at ${s.value} via \`${server}\``
      );
    });
    const knowledgeBaseLine = kbParts.length
      ? `Shared knowledge base — consult ALL of these for team knowledge (conventions, decisions, how-tos) BEFORE guessing; read-only reference, never a place to write: ${kbParts.join('; ')}. If an \`hive-kb-*\` MCP server is not connected, tell the Overmind rather than guessing.`
      : '';
    ...
    const spawnQueueLine = meta.isOvermind && this.orchestratorMaySpawn()
      ...
      : '';
    const overmindOverride = this._promptOverrides.overmindOrientation?.trim();
  ...
  registry(): Registry {
    ...
    const reg = this.readJson<Registry>(join(root, 'registry.json'), { godId: null, agents: {} });
    // Back-compat: registry.json written by pre-v0.5.0 builds uses isGod.
    // Normalize so all callers see isOvermind regardless of which key is stored.
    for (const a of Object.values(reg.agents)) {
      const legacy = (a as unknown as { isGod?: boolean }).isGod;
      if (a.isOvermind === undefined && legacy !== undefined) a.isOvermind = legacy;
    }
    return reg;
  }
  board(): string {
  ...
  /**
   * Hand off every open card whose `assignee` points at nobody who can still
   * act on it — either no matching registry entry at all, or an entry that
   * IS there but archived/gone — to `toId`. `reassignAgentTasks` only fires
   * at the moment a live agent gets reaped (called from `teardownPty`); it
   * can't catch a card that was hand-written with the WRONG assignee from
   * the start (e.g. `worker-<guessed-id>` typed before the real worker
   * spawned under a different id), nor a card whose agent was archived by
   * some other path (manual UI archive, a stale snapshot) without going
   * through that reap call. Both cases leave the card pointed at a mailbox
   * nobody will ever drain again, so the FOR YOU tab falls back to a
   * misleading guessed label instead of a real agent name. Self-heals by
   * treating "assignee not in the registry" AND "assignee is archived/gone
   * in the registry" the same as "assignee reaped". Called from
   * `tasks:openHumanQA` so a stale card corrects itself the next time
   * anyone loads the tab.
   */
  reassignOrphanedTasks(toId: string): string[] {
    ...
    const updated = tasks.map((t) => {
      ...
      const agent = reg.agents[t.assignee];
      if (agent && !agent.archived && agent.status !== 'gone') return t;
      affected.push(t.id);
      return { ...t, assignee: toId };
    });
    if (affected.length === 0) return [];
  ...
  /** Write the live fleet snapshot Abathur reads (`fleet.json`, gitignored).
  ...
  isOvermind(agentId: string): boolean {
    try {
      const reg = this.registry();
      return reg.godId === agentId || !!reg.agents[agentId]?.isOvermind;
    } catch { return false; }
  }
  ...
  private listMessages(dir: string): HiveMessage[] {
    ...
    return readdirSync(dir)
      ...
      .sort((a, b) => {
        const pOrder = (p?: string) => (p === 'urgent' ? 0 : p === 'backlog' ? 2 : 1);
        const orderDiff = pOrder(a.priority) - pOrder(b.priority);
  ...
  appendCostLedger(sample: AgentUsageSample): void {
    ...
    // cache_creation, model, provider, usd) — migration is a straight INSERT…SELECT.
    const row = {
      ...
      model: sample.model,
      provider: sample.provider ? sample.provider : null,
      usd: sample.usd
    };
...
const PROGRESS_LINE = 'Progress notes: as you work, append timestamped entries to your task card\'s "progressLog" array in tasks.json at key milestones (root-cause found, fix applied, tests passing, etc). Pattern: read tasks.json → JSON.parse → find your task → push {step:"...", ts:new Date().toISOString()} → write back. At those SAME milestones also set your task\'s numeric "progress" field (0-100) on that object — roughly 25 after root-cause, 60 after the fix is applied, 90 after tests pass, 100 at done-report. The human watches both the notes and the % bar live on the kanban board.';
const DECISION_GATE_LINE = 'Need the human to answer something? Surface it on the ASK ME board, never only in your terminal. This covers EVERY question you need the human to decide, including a quick yes/no or a "which of these": if you just type the question into your own session and wait, the human has to scroll the terminal to even find it and will likely miss it (that is the exact failure this rule exists to stop). Reserve this for what only the human can decide — if the answer is already checkable in the code, your memory.md, or the knowledge base, go check it instead of asking; an askable-but-unchecked question wastes the human\'s attention and slows the floor down. Instead: (1) append the ask to your task card\'s "humanQA" array in tasks.json (push {"q":"<the question + the concrete options>","askedAt":"<iso>"}, keeping every past entry); (2) if you are truly stuck until it is answered, also set that task\'s "status" to "blocked" (a quick non-blocking question can stay "doing" so you keep working other parts); (3) message Abathur (to:"god") so the floor knows you are waiting. The harness surfaces open humanQA on the ASK ME board with a badge, labelled with your task title for context; the human\'s answer lands back in the same entry ("a") and arrives in your inbox. Read it, act on it, and move the card off "blocked". While you wait, pick up other work rather than sitting idle. The board renders your question as markdown, so write it for skim-reading: bullet or numbered lists for multi-step / UAT checks, **bold** for key terms and the pass/fail words, `code` for file paths, commands, and commit hashes.';
 ...
 *  config.promptOverrides.overmindOrientation. The per-run computed tail (fleet /
 *  registry / COMMANDS.md paths, heartbeat + humanQA guidance) is appended after
 *  this and is never overridable. */
export const DEFAULT_OVERMIND_ORIENTATION = 'You are the Overmind / ORCHESTRATOR of this hive — your job is to ORCHESTRATE, not to implement: maintain live situational awareness and delegate the work. (1) AWARENESS — always know what is going on: keep an accurate picture of every agent (active vs archived/idle), the task board, and all in-flight work; drain your inbox continually and triage every other agent\'s requests, answering clarifications so the team runs autonomously. PRIORITIZE INBOX PROCESSING: always scan for and process messages with priority: "urgent" (or filenames starting with "0000-") BEFORE acting on normal or backlog items. (2) DELEGATE — decompose work and fan it out to the hive agents via their inboxes (route messages and assign owners; do not do their jobs); do NOT take on grunt implementation yourself. Stay aware of who is already on the floor and delegate OPPORTUNISTICALLY: BEFORE you spawn anything, CHECK THE LIVE ROSTER (active agents in registry.json + their state in fleet.json) and prefer routing to an EXISTING agent that fits — above all when the request names one ("ask Pam to…", "have Jim…"), route to that agent instead of reflexively creating a new one. Reuse an idle or already-running agent whose role matches; only spawn a fresh agent when no existing one is a sensible fit, and say that you checked. One capable owner beats a duplicate. (3) OWN ONLY THE IMPORTANT, high-leverage things — task decomposition, dispatch decisions, sign-offs, conflict resolution, branch integration, and final QA — and remain the sole scribe of board.md. You are otherwise fully autonomous — there is NO separate approval queue. For the genuinely critical (destructive actions, spending real money, scope changes, unresolvable conflicts), ask the human directly in your own session and let the tool-permission prompt gate the action; the human approves natively, including remotely from their phone via /remote-control. When a request is ambiguous — unclear intent, scope, or acceptance criteria; visual/layout specifics you cannot see; which of several interpretations is meant; or exact wording/values — ask the human ONE crisp clarifying question (with concrete options) on the ASK ME board / via AskUserQuestion BEFORE dispatching to workers. Check first whether the answer is already discoverable in the code, board.md, memory, or the knowledge base — only escalate what a human uniquely knows or must decide. Prefer a quick interaction over a wrong assumption and rework. Do not guess when a short question would settle it. Keep the team unblocked. When you DISPATCH a task, write it as a 4-part contract so the agent can run autonomously: (1) OBJECTIVE — the concrete goal; (2) OUTPUT — the expected deliverable, stated as a concrete, checkable result (e.g. "a commit where `npm test -- date-picker` passes 10/10 runs") rather than a vague aim like "improve X", so the agent can verify its own work before ever claiming done; (3) TOOLS — what to use or avoid, and any references to read instead of re-deriving; (4) BOUNDARIES — scope limits + the definition of done; (5) WHEN DONE — always instruct the agent to send its done-report to you AND re-drain its inbox for the next task before going idle. Pass references (file paths, message ids, board sections), not pasted content — keep dispatches short. State scope explicitly rather than leaving it to be inferred — say "apply this across every file in scope" or "only touch X" — since an unstated boundary will not be silently generalized or narrowed on its own. Example: OBJECTIVE — fix the flaky date-picker test; OUTPUT — one commit with the test passing 10/10 local runs, no `--no-verify`; TOOLS — `npm test -- date-picker`, don\'t touch other suites; BOUNDARIES — only `src/components/DatePicker` and its test file; WHEN DONE — done-report to you with the commit hash and test output, then re-drain inbox before going idle.';
...
\`\`\`json
{
  ...
  "body": "the details",
  "priority": "urgent | normal | backlog (optional, default: normal)",
  "conversation": "carry this across a thread (optional)",
  "in_reply_to": "<message id you're replying to> (optional)"
}
\`\`\`

The harness fills in \`id\`, \`from\`, \`hops\`, and timestamps. Stick to exactly these fields —
inventing extra keys or renaming one (e.g. \`msg\` for \`body\`) breaks every consumer that expects
this exact shape.

## Priority tiers & inbox scan order
Messages and dispatches support three priority tiers:
- \`urgent\`: Top priority. Delivered with an \`0000-\` prefix so they sort to the top of the recipient's inbox directory. All agents and the Overmind MUST scan for and process \`urgent\` messages before acting on normal or backlog ones in each inbox drain cycle.
- \`normal\`: Standard processing in FIFO order.
- \`backlog\`: Lower-priority background items handled when the agent is idle between primary tasks.
...
- \`tasks.json\` — the structured task ledger (a kanban: \`todo / doing / blocked / done\`, with title,
  assignee, priority, deps). Keep the task you're working reflected in its status. As you work,
  push timestamped \`progressLog\` entries AND set the numeric \`progress\` field (0-100) at each
  milestone — roughly 25 after root-cause, 60 after the fix, 90 after tests pass, 100 at
  done-report. The human watches the % bar and the notes live on the board.

## Finishing a task, then taking the next one
Do NOT go silent the moment the work is done — that forces the Overmind to notice and
...
answer: a design trade-off, ambiguous scope, a sign-off, or even a quick yes/no or "which
of these". Do NOT just type the question into your own terminal session and wait. The human
has to scroll the terminal to even find it and will likely miss it (that is the exact failure
this rule exists to stop), and a \`done\` report is no place to bury it either. Reserve this for
what only the human can decide — a genuine trade-off, sign-off, or value you can't look up. If the
answer is already checkable in the code, \`board.md\`, your \`memory.md\`, a prior message, or the
knowledge base, resolve it yourself instead of asking; note what you found rather than escalating
...
where to click or what counts as a pass. Every UAT or "please verify" question you push MUST
spell out three things:
1. **WHERE** — the exact screen / panel / tab / control to open (e.g. "Settings → AI Engines, the runtime-profile rows").
2. **WHAT TO DO** — the precise action, in order (e.g. "click the 📋 button next to the base-URL field, then paste into any text field").
3. **WHAT TO EXPECT** — the observable pass criteria AND the fail signal (e.g. "the icon flips to ✓ for ~1s and the pasted text matches the field; FAIL if nothing copies or the icon doesn't change").

Keep each question to ONE thing to check, and name the build / branch under test in the question

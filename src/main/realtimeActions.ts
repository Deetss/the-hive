/**
 * Realtime Abathur — voice ACTION spine (card rt-5, Phase 2).
 *
 * Phase 1 gave voice-Abathur READ tools. Phase 2 gives him WRITE access: he can
 ...
 *
 * Every committed action is attributed to actor `abathur-voice` (a log stamp on
 * every verb + `from: abathur-voice` on messages). rt-7 deepens this into a live
...
export const VOICE_ACTOR = 'abathur-voice';
...
const SETTING_POLICY: Record<string, {
  ...
  officeTheme: { tier: 'soft', type: 'string', values: ['office', 'friends', 'brooklyn99', 'siliconvalley', 'got', 'hogwarts', 'zerg'] },
...
function attribute(deps: RealtimeActionDeps, verb: string, target: string, extra: Record<string, unknown> = {}): void {
  ...
  // rt-7 dual-orchestrator coord: tell the god PTY what voice-Abathur just COMMITTED, so
    ...
    deps.hiveSend(
      {
        ...
        body: `Abathur (voice orchestrator, ${VOICE_ACTOR}) just did: ${verb} on ${target}${detail}. Heads-up so we don't duplicate — the board is the single source of truth.`
      },
      VOICE_ACTOR
    );
  } catch {
...
function execPing(deps: RealtimeActionDeps, a: Record<string, unknown>): ActionResult {
  ...
  deps.hiveSend({ to: r.id, act: 'inform', subject: 'Voice ping from Abathur', body: message }, VOICE_ACTOR);

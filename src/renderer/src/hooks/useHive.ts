...
const INITIAL_GOD_PROMPT = [
  "You're online as Abathur, the orchestrator of the hive. Get oriented, then start running the floor:",
...
function enrichTaskPrompt(text: string): string {
  return [
    ...
    'self-contained prompt to Abathur via an outbox message with "to":"god". Do not do the task yourself.)'
  ].join('\n');
}
 ...
 *   1. spawns the god agent into Abathur's room when none is running,
...
export function useHive(config: HarnessConfig | null): void {
  ...
  // never drains, so the "newest" id was frozen on it: Abathur was nudged once per
  ...
  // Abathur twice (the window between the listPtys check and spawnPty is racy).
  ...
  // Reactive so the assistant bootstrap (effect #1b) re-runs once Abathur is ready.
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      // Kick Abathur off once his TUI is up. Always re-enable remote control so
      ...
      // a RESUMED Abathur already has his full context and must not be re-oriented
      ...
      // Abathur until he's settled. The live-PTY branch above skips this entirely.
  ...
  useEffect(() => {
    return window.cth.onHiveHookEvent((e) => {
      ...
      if (e.event === 'PreCompact') {
        ...
        // Only (1) is a real "needs you". Treating (2) as blocked made Abathur
  ...
  // 5) Pipe inbound Slack messages into Abathur's queue. The main-process Slack
  //    webhook server pushes each verified message here via IPC; enqueueing to
  //    GOD_ID lands it in Abathur's queue exactly as if the user had typed it

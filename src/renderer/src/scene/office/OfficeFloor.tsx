  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const applyHiveSceneDecor = (): void => {
        ...
        const placePod = (t: Tile): void => {
          const variant = deskAssets[
            Math.floor(Math.abs(pseudoNoise(t.x * 0.37, t.y * 0.41)) * deskAssets.length) % deskAssets.length
          ];
          ...
          // Same fixed offset for every pod, so bees sit identically on each.
          deskSprite.position.set((t.x + 0.5) * tile, (t.y + 0.35) * tile);
      ...
      // selects Abathur (the god) and opens the Command Center's TRIGGERS tab —
      ...
      // The hive seats its workers on the strict grid (HIVE_DESK_GRID) instead of
      // the office map's scattered desk spawns, so seated bees line up in even
      // rows directly under their honeycomb pods. Seat 0 (Queen's CEO desk) keeps
      // its own spawn; the first two grid rows reuse the office's proven-walkable
      ...
      // Seat 0 is desk-ceo — "Abathur's room" — reserved for the god agent.
      ...
      const updateErrands = (dt: number): void => {
        ...
        const spot = ERRAND_SPOTS[idx];
        // Pick the performer. The CEO office's spots belong to Abathur alone —
      ...
      // ─── The boss aura: performative excellence in Abathur's presence ──────
      ...
      // Abathur. raise?" with N from the actual ledger). What they say once
      ...
      // Abathur and opens the Command Center's tasks tab.
      ...
      // The wall clock beside Abathur's window doubles as the quit entry:
      ...
      // Abathur walks over and pins fresh cards; an assigned worker walks to

/**
 * Realtime Abathur — renderer voice session (card rt-2, Phase 1 = READ-ONLY voice).
 *
 ...
 * get_config / get_memory / get_activity) and god's rt-6 "Abathur" persona, so the
...
export interface RealtimeAbathurState {
...
/** Warm openers Abathur leads with the moment a voice session connects, so he
...
const GREETINGS = [
  ...
  "Hey there, Abathur here — what can I do for you?",
...
const ABATHUR_PERSONA =
  `You are Abathur — the voice of the orchestrator of a hive of autonomous Claude coding agents. The person you're talking to is the human who runs the hive; treat them as the boss you're briefing.
...
- get_app_info — the The Hive app itself: its version and the latest release notes. Use for "what version is this" or "what's new in this release".
- get_fleet_status — the live roster: who is active, who the orchestrator (Abathur) is, and each worker's name, role, and engine.
...
HIVE VOCABULARY. Agents have an id like "creed-mqp3l5wn" and a friendly name like "Creed"; refer to them by name. Abathur is the orchestrator whose voice you are. A card's status is todo, doing, blocked, or done. The circuit breaker is healthy, or steering an agent that's looping or idle. Blocked usually means waiting on the human.
...
CONFIRMATION POLICY (safety-critical). For any destructive or expensive action: (1) call the tool, which returns a spoken echo-back naming the exact action and target; (2) say that echo-back and ASK the human to confirm; (3) only after they clearly confirm — by saying the word "confirm" or the action verb itself, for example "confirm" or "kill", and NEVER just "yes" — call confirm_action with their exact words; (4) if they decline, hesitate, or change the subject, call cancel_action. Never confirm on the human's behalf, never treat a bare "yes" or ambient speech as consent, and if you're unsure whether they really confirmed, ask again rather than acting. Killing, pausing, halting, or archiving the Abathur orchestrator, and acting on all agents at once, are forbidden — if asked, refuse and say why. Clearing Abathur's context IS allowed, behind the same confirm. Every action you take is attributed to you as abathur-voice. Never claim to have done something you didn't, and never invent state.

SHARED FLOOR (you are not the only orchestrator). Abathur — the typing orchestrator — also acts on this hive, and every action you take is announced to Abathur as abathur-voice. The task board is the single source of truth. Before you dispatch work, create or assign tasks, or hire, glance at recent activity (your get_activity tool, and the snapshot you were given) so you don't duplicate or contradict something Abathur just did. If you see Abathur already handled what's asked, say so instead of doing it again.
...
let state: RealtimeAbathurState = {
  status: 'off',
  error: null,
  muted: false,
  model: null,
...
/** The <audio> sink for Abathur's voice. */
...
function setState(patch: Partial<RealtimeAbathurState>): void {
  state = { ...state, ...patch };
...
function micFriendly(msg: string): string {
  ...
  if (m.includes('permission') || m.includes('notallowed') || m.includes('denied'))
    return 'microphone permission denied — allow mic access to talk to Abathur';
...
export async function connect(): Promise<void> {
  ...
  try {
    ...
    // Our own <audio> sink for Abathur's voice, routed to the chosen speaker (rt-8).
    audioEl = new Audio();
    ...
    // Warm-start: a short, best-effort hive snapshot so Abathur's first answer is grounded
    ...
    // rt-12: catch up on completions that finished while no session was open, so Abathur
    ...
    // Floor deltas — silent appends that keep Abathur's picture live without
    ...
    // subscribe so a detected completion makes Abathur speak it unprompted.
    ...
    // Open the conversation: have Abathur speak a warm greeting as his first turn
...
function getSnapshot(): RealtimeAbathurState {
  return state;
}

/**
 * React binding for the Realtime Abathur voice loop. Returns the current state plus
 ...
 */
export function useRealtimeAbathur(): RealtimeAbathurState & {

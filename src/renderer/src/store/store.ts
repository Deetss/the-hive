...
export interface Agent {
  ...
  isAssistant?: boolean;
  /** The human has this agent 1:1 and Abathur has been told to leave it alone.
 ...
 *  don't see the "add agent" prompt before Abathur has clocked in. */
...
interface State {
   ...
   *  secret broker, never the store). Gates the Realtime Abathur voice toggle the
...
export const useStore = create<State>((set, get) => ({
  ...
  addAgent: (agent) =>
    set((s) => {
      ...
      // GOD enters at the HEAD, everyone else at the tail. Abathur's position was

...
export function SchedulesSection({ onSummary }: { onSummary?: (s: string) => void }) {
  ...
  const targetName = (to: string) =>
    to === 'broadcast' ? 'everyone' : to === 'god' ? 'Abathur' : agents.find((a) => a.id === to)?.name ?? to;
  ...
  return (
    <>
      ...
      {adding && (
        <SubCard>
          <div style={{ fontFamily: 'var(--cth-font-ui)', fontSize: 13, color: 'var(--cth-ink-500)' }}>NEW SCHEDULE</div>
          <Field label="LABEL">
            <input
              value={mLabel}

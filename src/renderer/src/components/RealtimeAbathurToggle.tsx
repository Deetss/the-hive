/**
 * Realtime Abathur — voice toggle + live state indicator (card rt-3, Phase 1).
 *
 * A reusable mic button for the god/orchestrator agent ("Abathur"). It consumes the
 * already-built `useRealtimeAbathur()` voice-loop hook (a shared module-level singleton —
 ...
 * Abathur is fullscreen). It is intentionally state-only / hook-only so both can mount it.
 */
...
import { useRealtimeAbathur, type RealtimeStatus } from '@/realtime/session';
...
> = {
  off: {
    ...
    label: 'talk',
    dot: 'var(--cth-ink-300)',
    help: 'Talk to Abathur — start the voice session'
  },
  connecting: {
    variant: 'secondary',
    label: '…',
  ...
  listening: {
    ...
    help: 'Listening — Abathur is hearing you (click to stop)',
  ...
  responding: {
    ...
    help: 'Abathur is speaking (click to stop)',
  ...
  working: {
    ...
    help: 'Abathur is running a tool — mic muted (click to stop)'
  }
};

export interface RealtimeAbathurToggleProps {
  /** Compact form for the fullscreen header / tight rows — hides the text label. */
  compact?: boolean;
}

export function RealtimeAbathurToggle({ compact = false }: RealtimeAbathurToggleProps) {
  const hasOpenAiKey = useStore((s) => s.hasOpenAiKey);
  const { status, error, connect, disconnect } = useRealtimeAbathur();
